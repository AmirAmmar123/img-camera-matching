...
class CartesianCorrelation:
    def __init__(self, data_set: dict,  logger:Logger) -> None:
    ...
    def run(self):
        ...
        self.logger.info("Processing wavelet transform mapper...")
        ...
        for set_k in self.data_set.keys():
                ...
                for img_reader in self.data_set[set_k]:
                    ...
                    for i in range(img_reader.get_collection_size()):
                        try:
                            ...
                            self.logger.info(f"Reading image {img_reader.read_image_path(i)}")
                            if img_reader is not None:
                                ...
                                self.logger.info('Failed to read image')
                        except Exception as e:
                            self.logger.error(f"Error processing image {img_reader.read_image_path(i)}: {e}")
            ...
            try:
                ...
                self.logger.info(f"Processed and saved set {set_k}.json")
            except Exception as e:
                self.logger.error(f"Error saving data for set {set_k}: {e}")
